-- Corregir validación de SELECT en execute_safe_query
-- Problema: La función rechazaba queries con espacios/newlines al inicio incluso después de TRIM y REGEXP_REPLACE
-- Solución: Agregar LTRIM explícito después de normalizar espacios
...
BEGIN
    ...
    -- 🔥 NUEVO: Asegurar que no haya espacios al inicio después de normalizar
    ...
    IF NOT query_upper LIKE 'SELECT %' THEN
        RAISE EXCEPTION 'Solo se permiten consultas SELECT. Query recibido: "%"', LEFT(query_clean, 150);
    END IF;
...
COMMENT ON FUNCTION public.execute_safe_query IS 'Ejecuta consultas SQL SELECT de forma segura con validaciones y límites mejoradas. Incluye LTRIM para manejar espacios al inicio correctamente.';