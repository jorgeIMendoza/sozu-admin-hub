-- Drop and recreate the function with missing fields
...
BEGIN
  ...
  LEFT JOIN productos_servicios ps ON cc.id_producto_servicio = ps.id
  ...
  SELECT 
    ...
    cc.id_propiedad,
    cc.id_entidad_relacionada_dueno,
    ...
    -- NEW: dueno (nombre legal de la entidad relacionada)
    ent.nombre_legal as dueno,
    -- NEW: duplicate for frontend compatibility
    ed.nombre as estatus_disponibilidad_nombre,
    -- NEW: apartado_pagado - check if 'Apartado' concept is paid
    COALESCE(
      (SELECT ap.pago_completado 
       ...
       LIMIT 1),
      -- If no 'Apartado' concept exists, consider it paid if any payment exists
