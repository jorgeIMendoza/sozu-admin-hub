...
-- Solo ejecutar cuando es la PRIMERA asignación de cónyuge (OLD.id_conyuge IS NULL)
...
BEGIN
    -- Solo proceder si se está asignando un cónyuge por PRIMERA VEZ
    -- Esto evita la recursión infinita cuando se actualiza el cónyuge recíprocamente
    IF NEW.id_conyuge IS NULL OR (OLD.id_conyuge IS NOT NULL) THEN
        RETURN NEW;
    END IF;
...
'Agrega automáticamente al cónyuge como comprador en todas las cuentas de propiedades donde la persona es compradora activa. Solo se ejecuta en la primera asignación de cónyuge para evitar recursión infinita.';