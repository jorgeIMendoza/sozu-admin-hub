-- Actualizar el filtro de dueño para que NO dependa del proyecto y muestre todos los dueños
-- pero siga filtrándose cuando se selecciona un proyecto específico
UPDATE reportes 
SET filtros_configuracion = '[
  ...
  },
  {
    ...
    "tipo": "select",
    "requerido": false,
    "query_opciones": "SELECT DISTINCT pe.id, pe.nombre_legal as nombre FROM personas pe WHERE pe.activo = true AND (EXISTS (SELECT 1 FROM entidades_relacionadas er JOIN propiedades p ON p.id_entidad_relacionada_dueno = er.id WHERE er.id_persona = pe.id) OR EXISTS (SELECT 1 FROM entidades_relacionadas er JOIN productos_servicios ps ON ps.id_entidad_relacionada_dueno = er.id WHERE er.id_persona = pe.id)) ORDER BY pe.nombre_legal"
  },
  {
    "nombre": "tipo",
    "label": "Tipo",
