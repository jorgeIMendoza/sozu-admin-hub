...
-- Drop all versions of the function first, then recreate with consistent types
DROP FUNCTION IF EXISTS public.get_cuentas_cobranza_paginadas(integer, integer, text, text, text, text, text, text, text, integer[], text[], boolean, integer[], bigint[], text);
...
-- Recreate the function with integer[] for p_dueno_entity_ids (matching the frontend)
...
BEGIN
  ...
  SELECT 
    ...
    ec.nombre::text AS estatus_cuenta_nombre,
  ...
  GROUP BY cc.id, cc.clabe_stp, cc.precio_final, cc.fecha_compra, cc.activo, 
           cc.porcentaje_comision_venta, cc.valor_uma, cc.id_oferta, cc.id_estatus_cuenta,
           ec.nombre, prop.id, prop.numero_propiedad, prop.piso, proy.id, proy.nombre,
