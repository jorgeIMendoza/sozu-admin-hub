...
BEGIN
  ...
  LEFT JOIN personas vendedor ON o.id_persona_vendedor = vendedor.id
  WHERE cc.activo = p_activo
    ...
    AND (p_vendedor IS NULL OR COALESCE(vendedor.nombre_legal, '') ILIKE '%' || p_vendedor || '%')
    AND (p_estatus_ids IS NULL OR prop.id_estatus_disponibilidad = ANY(p_estatus_ids))
    AND (p_tipos IS NULL OR 
         (cc.id_oferta IS NOT NULL AND 'Propiedad' = ANY(p_tipos)) OR
         (cc.id_oferta_producto IS NOT NULL AND 'Producto' = ANY(p_tipos)))
  ...
  SELECT 
    ...
    cc.id_oferta_producto::INTEGER,
    CASE 
      WHEN cc.id_oferta IS NOT NULL THEN 'Propiedad'::TEXT
      WHEN cc.id_oferta_producto IS NOT NULL THEN 'Producto'::TEXT
      ELSE 'Servicio'::TEXT
    END AS tipo,
    ...
    vendedor.nombre_legal::TEXT AS nombre_vendedor,
    (
      SELECT jsonb_agg(jsonb_build_object(
