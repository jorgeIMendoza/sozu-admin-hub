...
BEGIN
  ...
  SELECT
    ...
    (SELECT pers.nombre_legal FROM personas pers 
     JOIN entidades_relacionadas er ON pers.id = er.id_persona 
     WHERE er.id = prop.id_entidad_relacionada_dueno AND er.activo = true LIMIT 1) AS dueno,
    ...
    COALESCE((SELECT jsonb_agg(jsonb_build_object('fecha_pago', p.fecha_pago, 'monto', p.monto)) 
      FROM pagos p WHERE p.id_cuenta_cobranza = cc.id AND p.activo = true AND p.id_metodos_pago = 1), '[]'::jsonb) AS cash_payments,
