...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- 1. Build filtered base into a temp table
  CREATE TEMP TABLE _inv_base ON COMMIT DROP AS
  SELECT
    p.id,
    p.numero_propiedad,
    p.numero_piso,
    ...
    mo.numero_recamaras,
    mo.numero_completo_banos,
    mo.numero_medio_bano,
  ...
  -- 2. Get total count
  SELECT count(*) INTO v_total_count FROM _inv_base;
  ...
  SELECT jsonb_build_object(
    ...
    'propiedades', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', b.id,
        'numero_propiedad', b.numero_propiedad,
        'numero_piso', b.numero_piso,
        'precio_lista', b.precio_lista,
        'm2_interiores', b.m2_interiores,
        'm2_exteriores', b.m2_exteriores,
        'proyecto_id', b.proyecto_id,
        'proyecto_nombre', b.proyecto_nombre,
        'edificio_nombre', b.edificio_nombre,
        'modelo_id', b.modelo_id,
        'modelo_nombre', b.modelo_nombre,
        'numero_recamaras', b.numero_recamaras,
        'numero_completo_banos', b.numero_completo_banos,
        'numero_medio_bano', b.numero_medio_bano,
        'bodegas_count', b.bodegas_count,
        'estacionamientos_count', b.estacionamientos_count,
        'estacionamientos_tipos', b.estacionamientos_tipos,
        'propiedad_imagenes', b.propiedad_imagenes
      ))
      FROM (
        SELECT * FROM _inv_base
        ORDER BY
    ...
    'modelo_imagenes', COALESCE((
      SELECT jsonb_object_agg(mid::text, imgs)
      FROM (
        SELECT DISTINCT b.modelo_id AS mid,
      ...
      ) unique_models WHERE imgs IS NOT NULL
    ), '{}'::jsonb),
    'esquemas_pago_proyecto', COALESCE((
      SELECT jsonb_object_agg(pid::text, schemes)
      FROM (
        SELECT DISTINCT b.proyecto_id AS pid,
          (SELECT jsonb_agg(jsonb_build_object(
      ...
      ) unique_projects WHERE schemes IS NOT NULL
    ), '{}'::jsonb),
    'filter_options', jsonb_build_object(
      'proyectos', COALESCE((SELECT jsonb_agg(DISTINCT proyecto_nombre ORDER BY proyecto_nombre) FROM _inv_base), '[]'::jsonb),
      'modelos', COALESCE((SELECT jsonb_agg(DISTINCT modelo_nombre ORDER BY modelo_nombre) FROM _inv_base), '[]'::jsonb),
      'recamaras', COALESCE((SELECT jsonb_agg(DISTINCT numero_recamaras ORDER BY numero_recamaras) FROM _inv_base WHERE numero_recamaras > 0), '[]'::jsonb),
      'niveles', COALESCE((SELECT jsonb_agg(DISTINCT numero_piso ORDER BY numero_piso) FROM _inv_base WHERE numero_piso IS NOT NULL), '[]'::jsonb)
    )
  ) INTO v_result;
