...
DROP TRIGGER IF EXISTS trg_agregar_conyuge_en_todas_cuentas ON personas;
DROP TRIGGER IF EXISTS trg_agregar_conyuge_como_comprador ON compradores;

-- 2. Crear trigger en tabla personas para ejecutar agregar_conyuge_en_todas_cuentas
CREATE TRIGGER trg_agregar_conyuge_en_todas_cuentas
  AFTER UPDATE OF id_conyuge ON personas
  FOR EACH ROW
  WHEN (OLD.id_conyuge IS DISTINCT FROM NEW.id_conyuge AND NEW.id_conyuge IS NOT NULL)
  EXECUTE FUNCTION agregar_conyuge_en_todas_cuentas();

-- 3. Crear trigger en tabla compradores para ejecutar agregar_conyuge_como_comprador
CREATE TRIGGER trg_agregar_conyuge_como_comprador
  AFTER INSERT ON compradores
  FOR EACH ROW
  WHEN (NEW.activo = true)
  EXECUTE FUNCTION agregar_conyuge_como_comprador();

-- 4. Modificar función agregar_conyuge_en_todas_cuentas para procesamiento bidireccional
...
BEGIN
    ...
    LOOP
        ...
        END IF;
        -- Si el cónyuge ya existe, no hacer nada (mantener porcentajes actuales)
    END LOOP;
    ...
    LOOP
        ...
        END IF;
        -- Si la persona original ya existe, no hacer nada (mantener porcentajes actuales)
    END LOOP;
