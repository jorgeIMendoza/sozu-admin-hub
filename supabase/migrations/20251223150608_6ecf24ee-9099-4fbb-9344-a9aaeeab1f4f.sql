-- Update report ID 5 to fix the "tipo" column logic
-- tipo should be 'Producto' when ofertas.id_producto IS NOT NULL, otherwise 'Propiedad'
UPDATE reportes
...
JOIN personas per ON cc.id_comprador = per.id
...
WHERE cc.activo = true
  ...
  AND (:id_proyecto::int IS NULL OR proy.id = :id_proyecto::int)
  AND (:id_dueno::int IS NULL OR ep_dueno.id_persona = :id_dueno::int)
